...
(function() {
    ...
    var hd = function() {
            // use for adSenseId
            var bcd = Math.round(2147483647 * Math.random());
            return bcd;
        },
        Bd = function() {
    ...
    var wb = /^(UA|YT|MO|GP)-(\d+)-(\d+)$/,
        ...
        Jc = function(a, b) {
            ...
            c = a.get(cc);
            if (e = (c = document.location[c ? "href" : "search"].match("(?:&|#|\\?)" + K("_ga").replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1") + "=([^&#]*)")) && 2 == c.length ? c[1] : "") a.get(bc) ? (c = e.indexOf("."), -1 == c ? J(22) : (d = e.substring(c + 1), "1" != e.substring(0, c) ? J(22) : (c = d.indexOf("."), -1 == c ? J(22) : (e =
            ...
            if (!a.get(Q))
                if (c = (c = O.gaGlobal && O.gaGlobal.vid) && -1 != c.search(/^(?:utma\.)?\d+\.\d+$/) ? c : void 0) J(17), a.data.set(Q, c);
                    ...
                    J(8);
                    c = O.navigator.userAgent + (document.cookie ? document.cookie : "") + (document.referrer ? document.referrer : "");
                    d = c.length;
                    for (e = O.history.length; 0 < e;) c += e-- ^ d++;
                    var sufixNum = Math.round((new Date).getTime() / 1E3);
                    var countRef = hd();
                    var pVal = La(c);

                    var prfixNum = countRef ^ pVal & 2147483647;



                    var retKey = [prfixNum, sufixNum];

                    a.data.set(Q, retKey.join("."))
                }
            mc(a)
        },
        Kc = function(a) {
